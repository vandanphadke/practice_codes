 ...
 * Output: 2
 * Explanation:
 * From starting index i = 0, we can jump to i = 2 (since A[2] is the smallest among A[1], A[2],
 * A[3], A[4] that is greater or equal to A[0]), then we can't jump any more.
 ...
 * In total, there are 3 different starting indexes (i = 1, i = 3, i = 4) where we can reach the
...
public class OddEvenJump {
    public int oddEvenJumps(int[] A) {
        ...
        indexMap.put(A[n-1], n-1);
        for (int i = n - 2; i >= 0; --i){
            int current = A[i];
            if (indexMap.containsKey(current)){
                even[i] = odd[indexMap.get(current)];
                odd[i] = even[indexMap.get(current)];
            }
            else {
                if (indexMap.lowerKey(current) != null){
                    even[i] = odd[indexMap.get(indexMap.lowerKey(current))];
                }
                if (indexMap.higherKey(current) != null){
                    odd[i] = even[indexMap.get(indexMap.higherKey(current))];
                }
            }
